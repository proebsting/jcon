...
abstract vValue Refresh();			//  ^x

// variable operations
abstract vString Name();			//  name(v)
abstract vInteger Args();			//  args(p)
abstract vValue Proc(long i);			//  proc(s, i)

// list operations
abstract vValue Push(vDescriptor v);		//  push(L, x)
abstract vValue Pull();				//  pull(L)
abstract vValue Pop();				//  pop(L)
abstract vValue Get();				//  get(L)
abstract vValue Put(vDescriptor v);		//  put(L, x)

// table/set operations
abstract vValue Key(iClosure c);		//  key(T)
abstract vValue Member(vDescriptor i);		//  member(X, x)
abstract vValue Delete(vDescriptor i);		//  delete(X, x)
abstract vValue Insert(vDescriptor i, vDescriptor val);  //  insert(X, x)

// set/cset operations
abstract vValue Complement();			//  ~x
abstract vValue Intersect(vDescriptor i);	//  x ** x
abstract vValue Union(vDescriptor i);		//  x ++ x
abstract vValue Diff(vDescriptor i);		//  x -- x
